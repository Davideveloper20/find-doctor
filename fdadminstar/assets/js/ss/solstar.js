...
class SolStar {
   ...
   * @return {[type]}     [description]
   */
  autocomplete(inp, arr, valctrl=false, cb=null, showempty=true) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < arr.length; i++) {
          ...
          if (arr[i].item.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            b.innerHTML += "<input type='hidden' data-id="+arr[i].id+" data-idx="+i+" value='" + arr[i].item + "'>";
            /*execute a function when someone clicks on the item value (DIV element):*/
            b.addEventListener("click", function(e) {
                /*insert the value for the autocomplete text field:*/
                inp.value = this.getElementsByTagName("input")[0].value;
                inp.dataset.id = this.getElementsByTagName("input")[0].dataset.id;
                inp.dataset.idx = this.getElementsByTagName("input")[0].dataset.idx;
                if(valctrl) {
                  valctrl.dataset.value = inp.value;
                  valctrl.dataset.idx =inp.dataset.idx;
                  valctrl.value = this.getElementsByTagName("input")[0].dataset.id;
                  $(valctrl).change();
                }
                if((typeof cb) == 'function') {
                  cb({id: inp.dataset.id, value: inp.value, idx: inp.dataset.idx, item: arr[inp.dataset.idx]});
                }
        ...
        if(a.childElementCount <= 0) {
          if(showempty) {
            b = document.createElement("DIV");
            b.id="nofound-autocomplete";
            /*make the matching letters bold:*/
            b.innerHTML = 'No Se encontraron Resultados';
            a.appendChild(b);
            inp.dataset.id = '';
            inp.dataset.idx = '';
            if(valctrl) {
              valctrl.dataset.value = '';
              valctrl.dataset.idx = '';
              valctrl.value = '';
              $(valctrl).change();
            }
  ...
  ajaxRequest(url, typeMethod, success, content=null, loader = true, progress=null, errorCb=null) {
    ...
    if((['POST', 'PUT', 'DELETE', 'PATCH'].indexOf(typeMethod)>=0)) {
    ...
    $.ajax({
      ...
      beforeSend: function (xhr) {
          ...
          if(me.Token && (['POST', 'PUT', 'DELETE', 'PATCH'].indexOf(typeMethod.toString().toUpperCase())>=0)) {
  ...
  /**
   * Cargador para dataTable
   * @param  {[Object]} config [Objeto con configuraciones]
   * @param  {[string]} config.el [id del elemeto table]
   * @param  {[function]} config.dataSource [funcion para el tratamiento de datos]
   * @param  {[function]} config.redrawFunction [funcion para aplicar a filar al re-dibujar, paginar o actualizar ]
   * @param  {[Object]} config.columnDefs [objeto con definicion de columnas ]
   * @param  {[Object]} config.columns [objeto con definicion de columnas a enlazar con la data ]
   * @param  {[Object | url]} config.language [objeto o url de configuraciones de lenguje ]
   * @param  {[url]} config.url [url desde donde se cargaran los datos (opcional) ]
   * @param  {[string]} config.typeMethod [define el tipo de solicitud para la url (opcional, se usa solo en el caso que se envie una url) ]
   * @return {[dataTable Instance]}     [instancia de dataTable]
   */
  dataTableReq(config) {
    ...
    if(((typeof config) == 'undefined') || !(config instanceof Object)) {
      console.log('Objeto de configuraciones para dataTableReq inválido, verifique');
      return false;
    }
    if(((typeof config.el) == 'undefined') || config.el == '') {
      console.log('Id de table para dataTableReq inválido, verifique');
      return false; 
    }
    if(((typeof config.columnDefs) == 'undefined') || !(config.columnDefs instanceof Object)) {
      console.log('columnDefs para dataTableReq inválido, verifique');
      //return false; 
    } else {
      options.columnDefs = config.columnDefs;
    }

    if(((typeof config.columns) == 'undefined') || !(config.columns instanceof Object)) {
      console.log('columns para dataTableReq inválido, verifique');
      return false; 
    }
    ...
    if((typeof config.language) == 'undefined') {
      options.language = {
        "sProcessing":     "Procesando... <i class=\"fa fa-spinner fa-spin fa-1x\" aria-hidden=\"true\"></i>",
        ...
        "sUrl":            "",
        "sInfoThousands":  ",",
        "sLoadingRecords": "Cargando... <i class=\"fa fa-sync-alt fa-spin fa-1x\" aria-hidden=\"true\"></i>",
        "oPaginate": {
          "sFirst":    "<i class=\"fa fa-angle-double-left fa-fw\" aria-hidden=\"true\"></i>Primero",
          "sLast":     "Último<i class=\"fa fa-angle-double-right fa-fw\" aria-hidden=\"true\"></i>",
          "sNext":     "Siguiente<i class=\"fa fa-angle-right fa-fw\" aria-hidden=\"true\"></i>",
          "sPrevious": "<i class=\"fa fa-angle-left fa-fw\" aria-hidden=\"true\"></i>Anterior"
        },
        "oAria": {
    ...
    } else if((typeof config.language) != 'undefined') {
      options.language = config.language;
    }

    if(((typeof config.url) != 'undefined') && config.url != '') {
      options.ajax = {
        url: $self.params.almacen+config.url,
        method: (config.typeMethod || "GET"),
        beforeSend: function (xhr) {
          /* Authorization header */
          if($self.getToken() != '') {
        ...
        error: function (xhr, error, thrown) {
          /*if($self.params.jqconfirm) {
            $self.jqc_alert('Error al Cargar dataTable: '+config.el,'<strong>Error:&nbsp;&nbsp;</strong>Ops! '+xhr.responseJSON.errors[0].message,'modern','fa fa-exclamation-triangle fa-1x','danger',{'Entendido!':function(e){}});
          } else if($self.params.bootstraptoast) {
            $self.btptoast('Error al Cargar dataTable: '+config.el,'<strong>Error:&nbsp;&nbsp;</strong>Ops! '+xhr.responseJSON.errors[0].message);
          } else if($self.params.alertify==true) {
            $selft.bigalert('<strong>Error:&nbsp;&nbsp;</strong>Ops! '+xhr.responseJSON.errors[0].message,'Error al Cargar dataTable: '+config.el);
          }*/
          console.log('Error al Cargar dataTable: '+config.el,'Error: Ops! ')//+xhr.responseJSON.errors[0].message);
        },
      };

      if((typeof config.dataSource) == 'function') {
        options.ajax.dataSrc = config.dataSource;
      }
    }

    return retDt = $(config.el).DataTable(options)
    .on( 'preDraw', function (evt) {
      if((typeof config.redrawFunction) == 'function') {
        config.redrawFunction(evt);
      }
    })
    .on( 'draw.dt', function (evt) {
      if((typeof config.redrawFunction) == 'function') {
        config.redrawFunction(evt);
      }
    })
    .on( 'page.dt', function (evt) {
      if((typeof config.redrawFunction) == 'function') {
        config.redrawFunction(evt);
      }
    })
    .on( 'error.dt', function ( e, settings, techNote, message ) {
      if($self.params.jqconfirm) {
        $self.jqc_alert('Error al Cargar dataTable: '+e.target.id,'<strong>Error:&nbsp;&nbsp;</strong>Ops! '+message,'modern','fa fa-exclamation-triangle fa-1x','danger',{'Entendido!':function(e){}});
      } else if($self.params.bootstraptoast) {
        $self.btptoast('Error al Cargar dataTable: '+e.target.id,'<strong>Error:&nbsp;&nbsp;</strong>Ops! '+message);
      } else if($self.params.alertify==true) {
        $selft.bigalert('<strong>Error:&nbsp;&nbsp;</strong>Ops! '+message,'Error al Cargar dataTable: '+e.target.id);
      } else {
        console.log('Error al Cargar dataTable: '+e.target.id,'Error: Ops! '+message);
      }
      console.log( 'Error: ' + message ); // for test purpose
      return true;
    });
  }
